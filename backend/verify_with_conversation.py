...
def run_test():
    ...
    print("\n--- 1.5. Seeding Conversation via Chat API ---")
    chat_payload = {
        "conversation_id": conversation_id,
        "message": "Hello, initialization."
    }
    try:
        requests.post(f"{API_URL}/api/chat/message", json=chat_payload, headers=HEADERS, timeout=600)
        print("Conversation seeded.")
    except Exception as e:
        print(f"Warning: Failed to seed conversation: {e}")
    ...
    try:
        ...
        resp = requests.post(f"{API_URL}/api/agents/workflows/execute", json=payload, headers=HEADERS, timeout=600)
